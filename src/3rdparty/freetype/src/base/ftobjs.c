...
#define FT_COMPONENT  objs
  ...
  FT_EXPORT_DEF(FT_Error)
  FT_Get_GlyphSlot_LoadFlags(FT_GlyphSlot slot, FT_Int32 *flags)
  {
      if (!slot || !slot->internal)
          return FT_THROW(Invalid_Face_Handle);

      *flags = slot->internal->load_flags;
      return FT_Err_Ok;
  }
...
#define FREETYPE_VER_FIXED  ( ( (FT_Long)FREETYPE_MAJOR << 16 ) | \
  ...
  FT_EXPORT_DEF( FT_Bool )
  FT_Get_Palette_Color( FT_Face       face,
                        FT_UInt       acolor_index,
                        FT_Byte       *r,
                        FT_Byte       *g,
                        FT_Byte       *b,
                        FT_Byte       *alpha)
  {
    ...
    if ( sfnt->get_palette_colr )
      return sfnt->get_palette_colr( ttface,
                                   acolor_index,
                                   r, g,
                                   b, alpha );
    else
